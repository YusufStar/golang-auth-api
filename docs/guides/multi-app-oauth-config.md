...
ALLOWED_REDIRECT_DOMAINS=localhost:3000,localhost:5173,localhost:8282,example.com,.example.com
...
```env
ALLOWED_REDIRECT_DOMAINS=localhost:3000,localhost:5173,localhost:8282,127.0.0.1:3000,127.0.0.1:5173
```
...
window.location.href = `/auth/google/login?redirect_uri=${encodeURIComponent(
  redirectUri
)}`;
```
...
if (urlParams.get("error")) {
  ...
  const provider = urlParams.get("provider"); // 'google', 'github'
...
ALLOWED_REDIRECT_DOMAINS=localhost:3000,localhost:5173,localhost:8282
DEFAULT_REDIRECT_URI=http://localhost:5173/auth/callback
```
