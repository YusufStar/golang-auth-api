...
curl -s http://localhost:8181/register > /dev/null
if [ $? -eq 0 ]; then
...
REGISTER_RESPONSE=$(curl -s -X POST http://localhost:8181/register \
...
LOGIN_RESPONSE=$(curl -s -X POST http://localhost:8181/login \
    -H "Content-Type: application/json" \
-d '{"email": "test@example.com", "password": "password123"}')
...
INVALID_LOGIN=$(curl -s -X POST http://localhost:8181/login \
    -H "Content-Type: application/json" \
...
PROTECTED_RESPONSE=$(curl -s http://localhost:8181/profile)
